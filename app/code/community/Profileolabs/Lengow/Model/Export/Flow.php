 ...
 *
 */
class Profileolabs_Lengow_Model_Export_Flow
{
	...
	protected function _getProductNode($product)
	{
		...
		$ParentIds = Mage::getResourceModel('catalog/product_model_type_configurable')->getParentIdsByChild($product->getId());
		echo "ParentIds:".$ParentIds;exit;
	...
	{
		$carrier = $this->getConfig()->getConfigData('default_shipping_method');
		$shippingPrice = 0;
		if(!empty($carrier))
		{
			
			$countryCode = $this->getConfig()->getConfigData('shipping_price_based_on');
			$shippingPrice = $this->getHelper()->getShippingPrice($product,$carrier,$countryCode);
		}
		
		if(!$shippingPrice)
		{
			$shippingPrice = $this->getConfig()->getConfigData('default_shipping_price');
		}
	...
	protected function getPercent($product)
	{
		...
		if($price == "0") { $price = round($product->getMinimalPrice(),2); }
	...
	public function getProductCollection()
	{
		if(is_null($this->_productCollection))
		{
			...
			$this->_productCollection->joinTable(
									            ...
									            'product_id=entity_id',
									            array('request_path'),
									            '{{table}}.category_id IS NULL AND {{table}}.is_system = 1 AND {{table}}.store_id = '.$this->getStoreId().'',
									            'left'
									        );
									        
			
			
			if($this->getConfig()->isFilteredByAttribute()){
				$this->_productCollection->addAttributeToFilter('lengow_product',array('eq' => 1));
			}
	...
	{
		return $this->getConfig()->getLimitProduct();
	}
	...
	protected function _validate($product)
	{
		...
		if(!$this->getConfig()->isExportSoldout() && !$product->isSaleable())
		{
			...
			$this->_errors[] = Mage::helper('profileolabs_lengow')->__("Le produit %s est hors stock. Il n'a pas été importé.",$product->getSku());
			return false;
		}
