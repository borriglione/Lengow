<?php
/**
 * Adminhtml log lengow grid
 *
 * @category   Lengow
 * @package    Profileolabs_Lengow_ManageOrders
 * @author kassim belghait
 */
class Profileolabs_Lengow_Block_Manageorders_Adminhtml_Log_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
	

    public function __construct()
    {
        parent::__construct();
        $this->setId('lengow_log_grid');
        $this->setUseAjax(true);
        $this->setDefaultSort('date');
        $this->setDefaultDir('DESC');
        $this->setSaveParametersInSession(true);
    }


    protected function _prepareCollection()
    {
        $collection = Mage::getModel("profileolabs_lengow/manageorders_log")->getCollection();
        
        $this->setCollection($collection);
        return parent::_prepareCollection();
    }

    protected function _prepareColumns()
    {

        $this->addColumn('id', array(
            'header'=> Mage::helper('profileolabs_lengow')->__('ID'),
            'width' => '80px',
            'type'  => 'text',
            'index' => 'id',
        ));


        $this->addColumn('date', array(
            'header' => Mage::helper('profileolabs_lengow')->__('Created at'),
            'index' => 'date',
            'type' => 'datetime',
            'width' => '100px',
        ));

        $this->addColumn('message', array(
            'header' => Mage::helper('profileolabs_lengow')->__('Message'),
            'index' => 'message',
        ));


        return parent::_prepareColumns();
    }

    public function getGridUrl()
    {
        return $this->getUrl('*/*/grid', array('_current'=>true));
    }

}
